...
a_crime_heinous1_sub = pd.read_csv('../csv/a_crime_heinous1_sub.csv', encoding='cp949')
c_crime_heinous1_sub = pd.read_csv('../csv/c_crime_heinous1_sub.csv', encoding='cp949')

ratio1 = a_crime_heinous1_sub['2019'][7:13]   # 성인재산
ratio2 = c_crime_heinous1_sub['2019'][7:13]   # 소년재산
ratio3 = a_crime_heinous1_sub['2019'][18:22]   # 성인흉악
ratio4 = a_crime_heinous1_sub['2019'][18:22]   # 소년흉악
ratio5 = a_crime_heinous1_sub['2019'][28:]   # 성인폭력
ratio6 = a_crime_heinous1_sub['2019'][28:]   # 소년폭력
...
def criminal_rate():
    ...
    ax1.set_yticklabels(['0', '20만명', '40만명', '60만명', '80만명', '100만명', '120만명', '140만명', '160만명', '180만명', '200만명'],
                        fontsize=40)
    bar1 = ax1.bar(x1 - 0.15, criminal_percent['총범죄자'], width=0.15, color='#C0C0C0', label='총범죄자')
    bar2 = ax1.bar(x1 + 0.00, criminal_percent['성인범'], width=0.15, color='#ADD8E6', label='성인범')
    bar3 = ax1.bar(x1 + 0.15, criminal_percent['소년범'], width=0.15, color='#8FBC8F', label='소년범')
    ax1.set_title("연도별 성인범죄자 비율 vs 소년범죄자 비율", fontsize=45, fontweight='bold')
    ...
    line1 = ax2.plot(x1, criminal_percent['성인범비율'], color='#4682B4', marker='o', markersize=15, linewidth=5,
                     label='성인범비율')
    line2 = ax2.plot(x1, criminal_percent['소년범비율'], color='#2E8B57', marker='o', markersize=15, linewidth=5,
                     label='소년범비율')
    ax2.set_ylabel('[범죄자비율]', fontsize=40)
    ax2.legend(ncol=2, loc='upper right', fontsize='30')

    for i, j in zip(x1, criminal_percent['성인범비율']):
        if j == criminal_percent['성인범비율'].max():
            plt.text(i, j - 1, j, va='top', ha='center', fontsize='40', color='red', fontweight='bold')
        elif j == criminal_percent['성인범비율'].min():
            plt.text(i, j - 1, j, va='top', ha='center', fontsize='40', color='blue', fontweight='bold')
        else:
            plt.text(i, j - 1, j, va='top', ha='center', fontsize='40')
    for i, j in zip(x1, criminal_percent['소년범비율']):
        if j == criminal_percent['소년범비율'].max():
            plt.text(i, j + 1, j, va='bottom', ha='center', fontsize='40', color='red', fontweight='bold')
        elif j == criminal_percent['소년범비율'].min():
...
# 형법범죄 중 재산, 흉악, 폭력범죄 발생 비율
    ...
    ax4.barh(x1 + 0.2, criminal_percent['성인흉악범죄비율'], left=criminal_percent['성인재산범죄비율'], label='성인흉악범죄비율', color='#6495ED',
             height=0.4)
    ...
    ax4.set_title("연도별 성인 및 소년 특정범죄비율 비교", fontsize=35, fontweight='bold', pad=20)
    ax4.set_xlabel('[특정범죄비율]', fontsize=30, labelpad=20)
    ax4.set_ylabel('[년도]', fontsize=30, labelpad=20)
    ax4.grid(alpha=0.5)
    ...
    ax5.barh(x1 - 0.2, criminal_percent['소년흉악범죄비율'], left=criminal_percent['소년재산범죄비율'], label='소년흉악범죄비율', color='#3CB371',
             height=0.4)
    ...
    for i, j in enumerate(np.arange(2013, 2020)):
        ...
        if str_val1 == criminal_percent['성인재산범죄비율'].max():
            plt.text(str_val1 / 2, j + 0.1, str_val1, fontsize='28', color='red', fontweight='bold')
        elif str_val1 == criminal_percent['성인재산범죄비율'].min():
            plt.text(str_val1 / 2, j + 0.1, str_val1, fontsize='28', color='yellow', fontweight='bold')
        else:
            plt.text(str_val1 / 2, j + 0.1, str_val1, fontsize='28')

        if str_val2 == criminal_percent['성인흉악범죄비율'].max():
            plt.text(str_val1 + str_val2 / 2, j + 0.1, str_val2, fontsize='28', color='red', fontweight='bold')
        elif str_val2 == criminal_percent['성인흉악범죄비율'].min():
            plt.text(str_val1 + str_val2 / 2, j + 0.1, str_val2, fontsize='28', color='yellow', fontweight='bold')
        else:
            plt.text(str_val1 + str_val2 / 2, j + 0.1, str_val2, fontsize='28')

        if str_val3 == criminal_percent['성인폭력범죄비율'].max():
            plt.text(str_val1 + str_val2 + str_val3 / 2, j + 0.1, str_val3, fontsize='28', color='red', fontweight='bold')
        elif str_val3 == criminal_percent['성인폭력범죄비율'].min():
            plt.text(str_val1 + str_val2 + str_val3 / 2, j + 0.1, str_val3, fontsize='28', color='yellow',
                     fontweight='bold')
        else:
            plt.text(str_val1 + str_val2 + str_val3 / 2, j + 0.1, str_val3, fontsize='28')

        plt.text(str_val1 + str_val2 + str_val3 + str_val4 / 2, j + 0.1, str_val4, fontsize='28')
    ...
    for i, j in enumerate(np.arange(2013, 2020)):
        ...
        if str_val1 == criminal_percent['소년재산범죄비율'].max():
            plt.text(str_val1 / 2, j - 0.3, str_val1, fontsize='28', color='red', fontweight='bold')
        elif str_val1 == criminal_percent['소년재산범죄비율'].min():
            plt.text(str_val1 / 2, j - 0.3, str_val1, fontsize='28', color='yellow', fontweight='bold')
        else:
            plt.text(str_val1 / 2, j - 0.3, str_val1, fontsize='28')

        if str_val2 == criminal_percent['소년흉악범죄비율'].max():
            plt.text(str_val1 + str_val2 / 2, j - 0.3, str_val2, fontsize='28', color='red', fontweight='bold')
        elif str_val2 == criminal_percent['소년흉악범죄비율'].min():
            plt.text(str_val1 + str_val2 / 2, j - 0.3, str_val2, fontsize='28', color='yellow', fontweight='bold')
        else:
            plt.text(str_val1 + str_val2 / 2, j - 0.3, str_val2, fontsize='28')

        if str_val3 == criminal_percent['소년폭력범죄비율'].max():
            plt.text(str_val1 + str_val2 + str_val3 / 2, j - 0.3, str_val3, fontsize='28', color='red', fontweight='bold')
        elif str_val3 == criminal_percent['소년폭력범죄비율'].min():
            plt.text(str_val1 + str_val2 + str_val3 / 2, j - 0.3, str_val3, fontsize='28', color='yellow',
                     fontweight='bold')
        else:
            plt.text(str_val1 + str_val2 + str_val3 / 2, j - 0.3, str_val3, fontsize='28')

        plt.text(str_val1 + str_val2 + str_val3 + str_val4 / 2, j - 0.3, str_val4, fontsize='28')

    plt.savefig('../img/specific_crime.png')
    # plt.show()
...
def criminal_to_crime():
    ...
    # plt.legend(ncol=3)
    # CD5C5C 4169E1
    for i, j in zip(x1, criminal_percent['성인범죄비율']):
        if j == criminal_percent['성인범죄비율'].max():
            plt.text(i, j + 1, j, va='bottom', ha='center', fontsize='20', color='red', fontweight='bold')
        elif j == criminal_percent['성인범죄비율'].min():
    ...
    for i, j in zip(x1, criminal_percent['소년범죄비율']):
        if j == criminal_percent['소년범죄비율'].max():
            plt.text(i + 0.05, j - 0.5, j, va='bottom', ha='left', fontsize='20', color='red', fontweight='bold')
        elif j == criminal_percent['소년범죄비율'].min():
